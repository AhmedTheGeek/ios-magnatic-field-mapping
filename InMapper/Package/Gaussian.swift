...
open class MultivariateGaussian {
    ...
    open func random(_ count: Int) throws -> [[Double]] {
        ...
        if (diagonalΣ) {
            ...
            var copyN4 = n;
            var copyN5 = n;
            dgesdd_(&jobZ, &copyN1, &copyN2, &A, &copyN3, &eigenValues, &u, &copyN4, &eigenVectors, &copyN5, &work, &lwork, &iwork, &info)
            if (info != 0 || work[0] < 1) {
            ...
            var copyN9 = n;
            var copyN10 = n;
            dgesdd_(&jobZ, &copyN6, &copyN7, &A, &copyN8, &eigenValues, &u, &copyN9, &eigenVectors, &copyN10, &work, &lwork, &iwork, &info)
            if (info < 0) {
                throw GaussianError.errorInSVDParameters
            }
            if (info > 0) {
